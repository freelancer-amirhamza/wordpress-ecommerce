...
(function ($) {
    function ResetDemoSettings() {
        ...
        $.post(ajaxurl, { action: 'grocery_ecommerce_reset_demo_import_settings' }, function () {
            location.reload();
        });
    }
...
(function ($, wp) {
    function ResetGlobalColor() {
        if (confirm("Are you sure you want to reset global color settings?")) {
            wp.customize.instance("grocery_ecommerce_first_theme_color").set("#0A472E");
			wp.customize.instance("grocery_ecommerce_second_theme_color").set("#05A845");
        }
    }
